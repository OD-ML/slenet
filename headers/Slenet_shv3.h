...
const dim3 cf_numBlocks(24, 6);
const dim3 cf_threadPerBlock(5, 5, 24);
...
__global__ void kernel_conv_filter(
    ...
    float weight[][5][5])
{
	int img_row = blockIdx.x;
	int ftr = blockIdx.y;	
	int img_col = threadIdx.z;
	int w_row = threadIdx.x;
	int w_col = threadIdx.y;
	__shared__ float sh_img[5][28];
	__shared__ float sh_weight[5][5];
	int idx = threadIdx.z*25 + threadIdx.x*5 + threadIdx.y;
	if (threadIdx.z == 0)
		sh_weight[w_row][w_col] = weight[ftr][w_row][w_col];
	if (idx < 140)
		sh_img[idx/28][idx%28] = input[img_row + idx/28][idx%28];
	__syncthreads();
	float sum = 0;
	if (threadIdx.x == 0 && threadIdx.y == 0) {
		for (int i=0; i<5; i++)
			for (int j=0; j<5; j++)	
				sum += sh_img[i][img_col+j] * sh_weight[i][j];
		preoutput[ftr][img_row][img_col] = sum;
	}
}
